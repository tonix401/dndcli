...
export function pruneStaleObjectives(gameState: IGameState): void {
    const currentChapter = gameState.getCurrentChapter();
    const narrativeCount = gameState.getNarrativeHistory().length;

    // Create a mapping to convert from StoryPaceKey to multiplier
    const paceMultipliers: Record<StoryPaceKey, number> = {
        FAST: 0.5,
        MEDIUM: 1.0,
        SLOW: 1.5,
    };
 ...
 */
export function enforceStoryRequirements(gameState: IGameState): {
    ...
    if (currentArc === "resolution") {
        const hasClimax = gameState
            .getChapters()
            .some((c: { arc: string }) => c.arc === "climax");
        if (!hasClimax) {
